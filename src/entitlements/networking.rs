...
pub struct Networking {
    ...
    /// If the CDN has an old version of the file, or doesn’t already have a copy of the file, it connects to your web server to obtain the latest version.
    ///
    /// While developing your app, if you use a private web server that’s unreachable from the public internet, you can use the alternate mode feature to bypass the CDN and connect directly to your private domain.
    ...
    /// * developer
    ///
    /// Specifies that only devices in developer mode can access the domain. In this mode, you can use any valid SSL certificate on your web server, including a certificate the system doesn’t trust.
    /// Make sure you don’t expose your users to security issues, such as man-in-the-middle attacks.
    /// As an added precaution, only apps signed with a development profile can use developer mode, and users must opt in on any device they use.
    /// * managed
    ///
    /// Specifies that only devices managed with a mobile device management (MDM) profile can access the domain.
