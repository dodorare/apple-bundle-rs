...
pub struct Networking {
    ...
    ///
    /// ### Note
    /// In macOS 11 and later and iOS 14 and later, apps request apple-app-site-association files
    /// from an Apple-managed content delivery network (CDN) dedicated to associated domains,
    /// instead of directly from your web server.
    /// If the CDN has an old version of the file, or doesn’t already have a copy of the file, it
    /// connects to your web server to obtain the latest version.
    ///
    ...
    /// * developer
    ///
    /// Specifies that only devices in developer mode can access the domain. In this mode, you can use any
    /// valid SSL certificate on your web server, including a certificate the system doesn’t trust.
    /// Make sure you don’t expose your users to security issues, such as man-in-the-middle attacks.
    /// As an added precaution, only apps signed with a development profile can use developer mode, and users
