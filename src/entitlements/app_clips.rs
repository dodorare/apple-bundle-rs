...
pub struct AppClip {
    ...
    ///
    /// Ensure that the application identifier for the App Clip uses the full app’s application identifier as its prefix, followed by a string.
    ...
    /// * App Clip
    #[serde(
        rename(serialize = "com.apple.developer.parent-application-identifiers"),
    ...
    ///
    /// When you code-sign your full app, Xcode embeds the App Clip in the full app and applies the com.apple.developer.on-demand-install-capable entitlement.
    /// Because of this behavior, the App Clip’s .entitlements file doesn’t include this entitlement if you open the file in Xcode’s Project navigator.
    ///
    /// To see the entitlement in the .entitlements file, first archive the full app, then export the App Clip for distribution as described in Distributing Your App Clip.
